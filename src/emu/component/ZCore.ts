...
export class ZCore {
    ...
    constructor(emuConfig: ZCoreSettings) {
        ...
        // add our peripherals
        this.parsePeripherals(emuConfig.peripherals).then(() => {
            //Wait for the Base Rom to load, then reset and go
            this.cpu.reset();
            // timer
            this.go();
        })
    ...
    async parsePeripherals(data: { ROM: ROM[] }) {
        for (let idx in data.ROM) {
            let item = data.ROM[idx];
            await this.mmap.addROM(item.name, item.start, item.size, item.uri);
        }
    }
